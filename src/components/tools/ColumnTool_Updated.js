...
const ColumnTool = ({
  ...
  return (
    <div className="column-tool-panel w-full h-full bg-white border border-gray-200 rounded-lg shadow-lg">
      ...
      {/* Parameters */}
      <div className="p-4 space-y-6 bg-white max-h-[70vh] overflow-y-auto">
              ...
              <button
                ...
                className={`flex-1 p-3 text-sm rounded-lg border transition-all ${
                  columnParams.shape === option.value
                    ? 'bg-studiosix-500 text-white border-studiosix-500 shadow-sm'
        ...
        <div>
          ...
          <div className="space-y-4">
              ...
              <input
                ...
                className={`w-full px-3 py-2 text-sm rounded-lg border transition-colors ${
                  validationErrors.height
                    ...
                    : 'border-gray-300 focus:border-studiosix-500'
                } focus:outline-none focus:ring-2 ${
                  validationErrors.height
                    ? 'focus:ring-red-200'
                    : 'focus:ring-studiosix-200'
                }`}
              />
              {validationErrors.height && (
            ...
            {/* Conditional dimensions based on shape */}
            {columnParams.shape === 'rect' ? (
              <div className="grid grid-cols-2 gap-4">
                ...
                <div>
                  ...
                  />
                  {validationErrors.depth && (
                    <p className="text-xs text-red-600 mt-1">{validationErrors.depth}</p>
                  )}
                </div>
              </div>
            ) : (
              /* Radius for circular columns */
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-1">
                  Radius (m)
                </label>
                ...
                />
                {validationErrors.radius && (
                  <p className="text-xs text-red-600 mt-1">{validationErrors.radius}</p>
                )}
              </div>
            )}
          </div>
          ...
          <div className="space-y-4">
            ...
            <div>
              ...
              <div className="flex items-center space-x-3">
                ...
                />
                <span className="text-sm font-medium text-gray-900 w-12">{columnParams.inclinationAngle}°</span>
              </div>
              {validationErrors.inclinationAngle && (
                <p className="text-xs text-red-600 mt-1">{validationErrors.inclinationAngle}</p>
              )}
            </div>
            ...
            <div>
              ...
              <div className="flex space-x-2">
                {inclinationAxisOptions.map((option) => (
                  <button
                    key={option.value}
                    onClick={() => handleParameterChange('inclinationAxis', option.value)}
                    className={`flex-1 p-2 text-sm rounded-lg border transition-all ${
                      columnParams.inclinationAxis === option.value
        ...
        <div>
          ...
          {selectedMaterial && (
            <div className="mt-3 p-3 bg-gray-50 rounded-lg">
              <p className="text-xs text-gray-600">
                <span className="font-medium">Density:</span> {selectedMaterial.density} kg/m³
              </p>
            </div>
          )}
        </div>
      ...
      {/* Actions */}
      <div className="p-4 border-t border-gray-200 flex space-x-3 bg-white rounded-b-lg">
        <button
          onClick={isEditing ? handleUpdate : handleCreate}
          disabled={!isValid || isCreating}
          className={`flex-1 flex items-center justify-center space-x-2 py-3 px-4 rounded-lg transition-all font-medium ${
            isValid && !isCreating
              ? 'bg-studiosix-500 hover:bg-studiosix-600 text-white shadow-sm'
        ...
        <button
          onClick={onCancel}
          className="px-6 py-3 rounded-lg transition-colors bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium"
        >
